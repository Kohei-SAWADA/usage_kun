...
この app は署名済みですが notarization はしていません(有料の Apple Developer アカウントが必要なため)。そのため初回起動時に macOS が「"UsageKun" にマルウェアが含まれていないことを確認できません」のような Gatekeeper 警告を表示します。これは notarize していない app では正常な動作で、download が壊れているわけではありません。開くには:

1. 一度 app を開こうとして、dialog を閉じます。
2. システム設定 > プライバシーとセキュリティ を開き、下にスクロールして UsageKun の項目の「このまま開く」を押します。
3. 続く dialog で確認します。
...
download した binary を信頼したくない場合は、下記の手順でソースから build してください。結果は同じで、Gatekeeper の承認も不要です。
...
新しい version の初回起動時に macOS が Gatekeeper 警告を再度出すことがあります。その場合は [Release ZIP からインストール](#release-zip-からインストール) と同じ方法で許可してください。Claude 公式 sync を有効にしている場合、更新後に macOS が Claude Code の Keychain access を再度確認することがあります。
